...
func (c *Controller) Subscribe(ctx context.Context, topic string) (extensions.BrokerChannelSubscription, error) {
	...
	msgChan := make(chan extensions.AcknowledgeableBrokerMessage, brokers.BrokerMessagesQueueSize)
...
func messagesHandler(c chan<- extensions.AcknowledgeableBrokerMessage) nsq.Handler {
	return nsq.HandlerFunc(func(message *nsq.Message) error {
		...
		c <- extensions.NewAcknowledgeableBrokerMessage(
...
func (noopAcknowledger) AckMessage() {}
func (noopAcknowledger) NakMessage() {}
